...
$artifactsRepoParam = ''
if ($Env:ARTIFACTS_REPO_OBJ) {
    $artifactsRepoParam = "artifactsRepo=$Env:ARTIFACTS_REPO_OBJ"
}
...
az deployment group create --template-file $imageBicepPath --name $Env:DEPLOYMENT_NAME --subscription $Env:SUBSCRIPTION_ID --resource-group $Env:RESOURCE_GROUP `
    ...
    ignoreBuildFailure=true imageBuildTimeoutInMinutes=$imageBuildTimeoutInMinutes $imageBuildProfileParam $artifactsRepoParam
